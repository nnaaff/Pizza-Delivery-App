...
def order(request):
    ...
    if request.method == 'POST':
        ...
        if filled_form.is_valid():
            note = 'Thank You for Ordering!! Your %s %s and %s pizza is on its way'%(filled_form.cleaned_data['size'],filled_form.cleaned_data['topping1'],filled_form.cleaned_data['topping2'])
            new_form = PizzaForm()
...
def pizzas(request):
    ...
    if request.method == 'POST':
        ...
        if(filled_formset.is_valid()):
            ...
            note = "Order was Received!! Thank You!!"
        else:
            note = "Order was not Received!! Please Try Again...."
